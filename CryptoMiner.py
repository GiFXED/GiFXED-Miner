...
# Set up logging
logging.basicConfig(filename='crypto_miner.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

def mine(queue, block_number, transactions, previous_hash, start_nonce, end_nonce):
    logging.info(f'Starting mining from nonce {start_nonce} to {end_nonce}')
    for nonce in range(start_nonce, end_nonce):
        data = str(block_number) + str(transactions) + str(previous_hash) + str(nonce)
        hash_value = hashlib.sha256(data.encode()).hexdigest()
        if hash_value.startswith('0'*6):
            logging.info(f'Block mined with nonce value: {nonce}, hash: {hash_value}')
            queue.put(hash_value)
            return
    logging.info(f'Mining completed for range {start_nonce} to {end_nonce} without success.')
...
def send_mined_block(queue, block_number, transactions, previous_hash, miner_address, mining_currency, crypto_address):
    while True:
        ...
        logging.info(f'Sending mined block to {mining_pool_url}, hash: {hash_value}')
        requests.post(mining_pool_url, json={
        ...
        logging.info(f'Block successfully sent to {mining_pool_url}')

def mine_blocks(num_threads, block_number, transactions, previous_hash, miner_address, mining_currency, crypto_address):
    logging.info(f'Starting block mining with {num_threads} threads')
    queue = Queue()
    threads = []
